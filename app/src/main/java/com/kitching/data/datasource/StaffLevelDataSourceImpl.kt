...
class StaffLevelDataSourceImpl(private val db: FirebaseFirestore = FirebaseFirestore.getInstance()):
    ...
    override suspend fun getStaffLevel(staffLevelId: String): Result<StaffLevel> {
        return runCatching {
            db.collection(COLLECTION_STAFF_LEVEL).whereEqualTo("id", staffLevelId).get().await()
                .toObjects(StaffLevel::class.java).first()
        }
    }
